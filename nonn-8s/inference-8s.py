...
def exit_handler():
    ...
    global time_send
    for x in time_send:
        print("send  to  sock", x, ": mean=%.3e += (%.1e)" % (statistics.mean(time_send[x]), statistics.variance(time_send[x])), ", median=%.3e" % (statistics.median(time_send[x])))
    for x in time_recv:
        print("recv from sock", x, ": mean=%.3e += (%.1e)" % (statistics.mean(time_recv[x]), statistics.variance(time_recv[x])), ", median=%.3e" % (statistics.median(time_recv[x])))
...
def connects( socks, servers ):
    for dev in servers:
        try:
            ...
            print( dev + " not found(" + servers[dev]['ip'] + "," + str(servers[dev]['port']) + ")")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config",     help="Configuration", type=str)
    ...
    extr_node = [('g0',34),('g1',46),('g2',34),('g3',46),('g4',34),('g5',46),('g6',34),('g7',46)]
    for batch_idx, (inputs, targets) in enumerate(cifar):
        ...
        if batch_idx >= 0:
            for name,_ in extr_node:
                client_send( normed, socks[name], label=name )
            if args.time_meas:
        ...
        if batch_idx >= 0:
            x2_pool = []
            for name, sz in extr_node:
                x2_tmp = client_recv( socks[name], label=name )
                x2_pool.append( torch.Tensor(x2_tmp).view(1,sz,8,8) )
            if args.time_meas:
            ...
            x2 = torch.cat( tuple(x2_pool), 1 )
            if "cat" in time_recv:
                time_recv["cat"].append(time.time()-time_start_cat)
            else:
                time_recv["cat"] = [time.time()-time_start_cat]
