...
class conv_chain(nn.Module):

    def __init__(self, n=1000):
        super(conv_chain,self).__init__()
        self.conv = nn.Conv2d(3,3,kernel_size=3,padding=1,bias=False)
        randn_weight = torch.ones_like(self.conv.weight)/27
        zmean_weight = randn_weight# - randn_weight.mean()
        self.conv.weight = torch.nn.Parameter(zmean_weight)
        # print("w.shape = ", self.conv.weight.shape )
    def forward(self, x):
        for i in range(3000):
            x = self.conv(x)
        return x
...
if __name__ == '__main__':
    ...
    print("conv_chain input = ", x[0][0][0])
    c = conv_chain()
    x = c(x)
    print("x = ", x[0][0][0])
    ...
    torch.onnx.export(c, dummy_input, "conv_chain.onnx", verbose=True)
#     if args.to_onnx:
#         from torch.autograd import Variable
#         import torch.onnx
